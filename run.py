...
def main(args):
    ...
    dataset = LinkPropPredDataset(name = 'bank', meta_dict = my_meta_dict)
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step % args.log_steps == 0:
                ...
                training_loss.extend([i['loss'] for i in training_logs])
                plt.clf()
                plt.plot(training_loss)
                plt.xlabel('steps')
                plt.ylabel('train_loss')
                plt.title('TRAIN LOSS')
                plt.savefig(os.path.join(args.save_path or args.init_checkpoint, 'train_loss.png'))
                training_logs = []
            ...
            if args.do_valid and step % args.valid_steps == 0 and step > 0:
                ...
                mrr_valid.append(metrics['mrr_list'])
                plt.clf()
                plt.plot(range(args.valid_steps, len(mrr_valid)*args.valid_steps+1 ,args.valid_steps), mrr_valid)
                plt.xlabel('steps')
                plt.ylabel('mrr_valid')
                plt.title('MRR VALID')
                plt.savefig(os.path.join(args.save_path or args.init_checkpoint, 'mrr_valid.png'))
    ...
    if args.evaluate_train:
        ...
        small_train_triples = {}
        indices = np.random.choice(len(train_triples['head']), args.ntriples_eval_train, replace=True)
